...
このプロジェクトは、**クレジットカード明細の月次 CSV を結合し、手動で「対象チェック」「備考」を付けたあと、freee に取り込める形式の CSV を生成する** Python ツールです。
...
5. **freee 用 CSV 出力**: `freee_import.csv` を freee の経費インポートに利用可能
...
- 次のファイルが **リポジトリ直下** に出力されます。
  - **freee_import.csv**: freee の経費インポート用
  - **need_review.csv**: 勘定科目が「要確認」の行だけ
  - **store_summary_with_account.csv**: 店名×勘定科目ごとの集計

freee には **freee_import.csv** をインポートして利用します。要確認の行は **need_review.csv** で確認し、必要なら `rules.yaml` を修正して再実行できます。
...
| freee_import.csv | freee 経費インポート用CSV | しない |
...
- **生成物**: 上記の combined_*.csv / freee_import.csv などもコミットしません。
